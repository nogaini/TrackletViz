...
- **vLLM** *(optional)* - to generate MLLM cluster descriptions at index time; see Section 2d
...
### 2d. (Optional) Start vLLM for Cluster Descriptions

If you want cluster cards to include natural-language descriptions, start a vLLM server before running the indexer. The indexer will call it automatically and store the descriptions in Qdrant. If vLLM is not running, indexing proceeds normally and cluster cards simply have no descriptions.
...
Start the server (before indexing):

```bash
vllm serve Qwen/Qwen2-VL-2B-Instruct --port 8001
```

Other recommended models for a single 24 GB GPU:

| Model | VRAM (FP16) |
|-------|-------------|
| `Qwen/Qwen2-VL-2B-Instruct` | ~8 GB |
| `microsoft/Phi-3.5-vision-instruct` | ~12 GB |

The server endpoint and model name are configurable via the `mllm:` section in `default.yaml` (see Section 3).
...
The indexing pipeline runs 21 steps (plus optional MLLM sub-steps if vLLM is running) and stores results in Qdrant. Run it from the `backend/` directory:
...
| `local-embeddings` | Clears the embeddings and preprocessed-embeddings checkpoints and re-runs steps 8–13 (re-embeds all tracklets, re-preprocesses, re-clusters, re-stores) |
| `local-clustering` | Clears the tracklet-metadata checkpoint and re-runs steps 9–13 (re-clusters, re-selects representatives, re-generates MLLM descriptions, re-stores) without re-running detection or re-embedding |
...
| `global-clustering` | Clears the clips-indexed checkpoint and re-runs steps 18–21 (re-clusters global clips, re-selects representatives, re-generates MLLM descriptions, re-stores) without re-embedding clips |

Multiple components can be combined in a single command. Pipeline resets (`local-embeddings`, `local-clustering`, `global-embeddings`, `global-clustering`) suppress redundant surgical work for their downstream assets.
...
| 8.5 | Embedding preprocessing | Optional: L2-normalize and/or UMAP-reduce to N dims before clustering (controlled by `clustering.preprocess` in config); no-op if both flags are off |
| 9 | UMAP + HDBSCAN | Reduces to 2D; clusters in preprocessed (or original) high-dim space |
...
| *(MLLM)* | Local cluster descriptions | If vLLM is reachable: sends up to 5 representative thumbnails + stats per cluster to the MLLM; stores one-sentence description on each `ClusterStatistics` record and a meta-summary on `VideoMetadata`; skipped silently if vLLM is unavailable |
...
| *(MLLM)* | Global cluster descriptions | If vLLM is reachable: sends representative clip thumbnails + derived stats per cluster; stores descriptions and a meta-summary in the video metadata payload via `patch_video_metadata`; skipped silently if vLLM is unavailable |
...
2. *(Optional)* **vLLM** - `vllm serve Qwen/Qwen2-VL-2B-Instruct --port 8001` — start before indexing if you want MLLM cluster descriptions; skip to index without them
3. **Index video(s)** - run the indexing pipeline for each video you want to explore
4. **Backend API** - `cd backend && export XLA_PYTHON_CLIENT_PREALLOCATE=false && .venv/bin/python -m uvicorn api.main:app --host 0.0.0.0 --port 8000` (the `export` keeps the long-lived API server from preempting all GPU memory)
...
- If MLLM descriptions were generated at index time, a **meta-summary banner** at the top of the tab explains what the clusters collectively distinguish from each other (e.g., "Clusters 0–2 capture pedestrian foot traffic while clusters 3–4 represent faster vehicles")
- Each cluster appears as a card with color-coded border matching the scatter plot
- Cards show: cluster ID (or "Noise" for cluster −1), member count, average speed, class distribution percentages, an optional **MLLM description** (one-sentence summary of the cluster's dominant activity or object type), and thumbnail representatives
...
- If MLLM descriptions were generated at index time, a **meta-summary banner** at the top explains what the global clusters collectively distinguish
- Each global clip cluster appears as a card with a full-scene representative thumbnail
- Cards show: cluster ID (or "Noise" for cluster −1), member count, an optional **MLLM description**, and representative clip thumbnails
...
| `GET` | `/api/global-clips/{video_id}/cluster-stats` | Cluster statistics for global clips; response shape: `{clusters: [{cluster_id, member_count, representative_clip_ids, description?}], meta_summary?}` |
...
**Cluster descriptions not appearing**
- Descriptions are generated at index time, not on demand — you must re-index after starting vLLM
- Confirm vLLM is running before the indexer starts: `curl http://localhost:8001/v1/models`
- Check indexer logs for `[MLLM] Local descriptions: N/M clusters` and `[MLLM] Global descriptions: N/M clusters`
- If `mllm.enabled: false` in `default.yaml`, descriptions are skipped regardless of vLLM state
- vLLM must be started with the model matching `mllm.model` in `default.yaml`
...
│   │   └── mllm.py             # vLLM client for MLLM cluster descriptions
│   ├── api/                    # FastAPI backend
